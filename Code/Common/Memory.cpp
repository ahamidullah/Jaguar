...
Chunk_Header *make_memory_chunk() {
	Chunk_Header *chunk = (Chunk_Header *)PlatformAllocateMemory(CHUNK_DATA_PLUS_HEADER_SIZE);
...
void InitializeMemory(GameState *game_state) {
	platform_page_size = PlatformGetPageSize();
	memory_chunks = make_memory_chunk();
	active_memory_chunk = memory_chunks;
	memory_block_free_head = NULL;

	game_state->frameArena = make_memory_arena();
