...
void Create_Material_Pipelines(Render_API_Context *context, GPU_Shader shaders[GPU_SHADER_COUNT], GPU_Render_Pass scene_render_pass, GPU_Pipeline *pipelines) {
	{
		...
		GPU_Pipeline_Vertex_Input_Attribute_Description AttributeDescriptions[] = {
			{
		...
		GPU_Pipeline_Vertex_Input_Binding_Description BindingDescriptions[] = {
			{
				.binding = GPU_VERTEX_BUFFER_BIND_ID,
				.stride = sizeof(V3),
		...
		GPU_Dynamic_Pipeline_State DynamicStates[] = {
			(GPU_Dynamic_Pipeline_State)GPU_DYNAMIC_PIPELINE_STATE_VIEWPORT,
			(GPU_Dynamic_Pipeline_State)GPU_DYNAMIC_PIPELINE_STATE_SCISSOR,
		};
		GPU_Pipeline_Description pipeline_description = {
			...
			.viewport_height = (f32)window_width,
			...
			.vertex_input_attribute_descriptions = AttributeDescriptions,
			.vertex_input_binding_count = 1,
			.vertex_input_binding_descriptions = BindingDescriptions,
...
GPU_Shader_Descriptor_Sets Create_Descriptor_Sets_For_Shader(Render_Context *context, u32 swapchain_image_count, GPU_Descriptor_Pool descriptor_pool, GPU_Shader_ID shader_id) {
	...
	case RUSTED_IRON_SHADER: {
		...
		sets.buffer = Create_GPU_Device_Buffer(context, sizeof(M4) * swapchain_image_count + 0x100 * swapchain_image_count, (GPU_Buffer_Usage_Flags)(GPU_UNIFORM_BUFFER | GPU_TRANSFER_DESTINATION_BUFFER));
	} break;
	default: {
