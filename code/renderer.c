...
struct Skeleton_Asset {
	u8 joint_count;
	aiString *joint_names; // @TODO: Get rid of this aiString.
	Skeleton_Joint_Skinning_Info_ *joint_skinning_info;
...
void initialize_assets(Game_Assets *assets) {
	assets->animations.instances = allocate_array(assets->arena, Animation_Instance, 100);
	assets->animations.lookup = allocate_array(assets->arena, u32, 100);

	assets->models.instances = allocate_array(assets->arena, Model_Instance, 100);
	assets->models.lookup = allocate_array(assets->arena, u32, 100);
}
