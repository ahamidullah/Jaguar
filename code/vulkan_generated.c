...
void Render_API_Load_Shaders(Render_API_Context *context, GPU_Shader shaders[GPU_SHADER_COUNT]) {
	SHADER_SPIRV_FILEPATHS[RUSTED_IRON_SHADER][GPU_VERTEX_SHADER_STAGE] = "build/shaders/binaries/rusted_iron_vert.spirv";
	SHADER_SPIRV_FILEPATHS[RUSTED_IRON_SHADER][GPU_FRAGMENT_SHADER_STAGE] = "build/shaders/binaries/rusted_iron_frag.spirv";
	...
	for (s32 i = 0; i < GPU_SHADER_COUNT; i++) {
		for (s32 j = 0; j < GPU_SHADER_STAGE_COUNT; j++) {
			...
			VkShaderModuleCreateInfo shader_module_create_info = {
				...
				.codeSize = (size_t)spirv.size,
				.pCode = (u32 *)spirv.data,
			};
			shaders[i].stages[shaders[i].stage_count] = (GPU_Shader_Stage_Flags)j;
